...
const fechaFormateada = currentDate.getFullYear() + '-' + ('0' + (currentDate.getMonth() + 1)).slice(-2) + '-' + ('0' + currentDate.getDate()).slice(-2) + ' ' + ('0' + currentDate.getHours()).slice(-2) + ':' + ('0' + currentDate.getMinutes()).slice(-2) + ':' + ('0' + currentDate.getSeconds()).slice(-2);
const fechaFormateada2 = previousDate.getFullYear() + '-' + ('0' + (previousDate.getMonth() + 1)).slice(-2) + '-' + ('0' + previousDate.getDate()).slice(-2) + ' ' + ('0' + previousDate.getHours()).slice(-2) + ':' + ('0' + previousDate.getMinutes()).slice(-2) + ':' + ('0' + previousDate.getSeconds()).slice(-2);
...
async function makeRequest(retryCount = 0, sessionid) {
  ...
  try {
    ...
    params.append('filter', `{"policytype":["policy","security-policy","local-in-policy","local-in-policy6","sniffer"]}`);
...
const columns = [
  ...
  { name: 'srcintf', align: 'left', label: 'Interface origen', field: 'srcintf' },
  { name: 'dstintf', align: 'center', label: 'Interfaz destino', field: 'dstintf' },
...
<template>
      ...
      <q-input filled v-model="form.from" label="Desde">
        ...
        <template v-slot:append>
          <q-icon name="access_time" class="cursor-pointer">
            <q-popup-proxy cover transition-show="scale" transition-hide="scale">
              <q-time v-model="form.from" mask="YYYY-MM-DD HH:mm" format24h with-seconds>
                <div class="row items-center justify-end">
                  <q-btn v-close-popup label="Close" color="primary" flat />
                </div>
              </q-time>
  ...
  <q-table flat dense bordered :rows="result" :columns="columns" row-key="policyid" :loading="loading"
    ...
    <template v-slot:body="props">
      <q-tr :props="props">
        ...
        <q-td key="rcvdbyte" :props="props"> <strong>{{ Math.round(props.row.rcvdbyte / 1000000) }}</strong> MB</q-td>
        <q-td key="sentbyte" :props="props"> <strong>{{ Math.round(props.row.sentbyte / 1000000) }}</strong> MB</q-td>

        <q-td key="bytes" :props="props"> <strong>{{ Math.round(props.row.bytes / 1000000) }} MB</strong></q-td>
      </q-tr>
    </template>
    <template v-slot:bottom-row>
      <q-tr class="bg-primary text-white">
        ...
        <q-td class="text-center">
          {{ Math.round(sumaRecibido / 1e+9) }} GB
        </q-td>
        <q-td class="text-center">
          {{ Math.round(sumaEnviado / 1e+9) }} GB
        </q-td>
        <q-td class="text-center">
          {{ Math.round(sumaTotal / 1e+9) }} GB
        </q-td>
      </q-tr>
    </template>
  </q-table>
